...
Cypress.Commands.add("findTable", (headers, checkCount) => {
    // Wait for the right table to show up.
    for (let h of headers) {
        cy.get("table thead th").should("contain.text", h)
    }
    // Now check the tables for the one with the right row.
    cy.get("table").then(($tbl) => {
        let headersLowercase = headers.map((h) => h.toLowerCase().trim())
        let tableFound = false
